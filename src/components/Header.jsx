...
export default function ProfileHeader({
  ...
  useEffect(() => {
    if (showModal) {
      ...
      setIsEditing(false);
      setPhotoUrl(''); // Reset URL when modal opens
    }
  ...
  const handleUrlChange = e => {
    ...
    setEdited(ed => ({ ...ed, photo: e.target.value }));
  };
  ...
  const saveChanges = async () => {
    try {
      ...
      const response = await axios.patch(endpoint, edited);
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        {actions.length > 0 && (
          <div className="space-x-2">
            {actions.map(a => (
              <button key={a.name} className="btn btn-secondary" onClick={() => onAction(a.name)}>
                {a.label}
              </button>
            ))}
          </div>
        ...
        <div className="edit-modal-overlay" onClick={toggleModal}>
          <div className="edit-modal" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
                ...
                <div className="relative">
                  ...
                  {isEditing && (
                    <label className="absolute bottom-0 right-0 bg-green-600 text-white rounded-full p-2 cursor-pointer">
                      <input type="file" accept="image/*" onChange={handlePhotoChangeModal} className="hidden" />
                    </label>
                  )}
                </div>
                <div>
              ...
              <div>
                {!isEditing ? (
                  <button onClick={() => setIsEditing(true)} className="btn btn-primary mr-2 text-sm">
                    Edit
                  </button>
                ) : (
                  <>
                    <button onClick={saveChanges} className="btn btn-primary mr-2 text-sm">
                      Save
                    </button>
